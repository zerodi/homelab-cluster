...
CHART_PIN = re.compile(r'^\s{2}(?P<key>[a-z0-9_]+):\s+"(?P<version>[^"]+)"\s*$')
...
ARGO_CONSUMERS = (
    ArgoConsumer(
        "authentik",
        "argocd/platform/identity/applications/authentik/app.yaml",
        "authentik",
    ),
    ArgoConsumer(
        "postgresql",
        "argocd/platform/identity/applications/authentik/postgresql.yaml",
        "postgresql",
    ),
    ArgoConsumer(
        "redis",
        "argocd/platform/identity/applications/authentik/redis.yaml",
        "redis",
    ),
    ArgoConsumer(
        "forgejo",
        "argocd/platform/delivery/applications/forgejo/app.yaml",
        "forgejo",
    ),
    ArgoConsumer(
        "postgresql",
        "argocd/platform/delivery/applications/forgejo/postgresql.yaml",
        "postgresql",
    ),
    ArgoConsumer(
        "valkey",
        "argocd/platform/delivery/applications/forgejo/valkey.yaml",
        "valkey",
    ),
    ArgoConsumer(
        "harbor",
        "argocd/platform/delivery/applications/harbor/app.yaml",
        "harbor",
    ),
    ArgoConsumer(
        "postgresql",
        "argocd/platform/delivery/applications/harbor/postgresql.yaml",
        "postgresql",
    ),
    ArgoConsumer(
        "valkey",
        "argocd/platform/delivery/applications/harbor/valkey.yaml",
        "valkey",
    ),
    ArgoConsumer(
        "woodpecker",
        "argocd/platform/delivery/applications/woodpecker/app.yaml",
        "woodpecker",
    ),
    ArgoConsumer(
        "velero",
        "argocd/platform/storage/applications/velero/app.yaml",
        "velero",
    ),
    ArgoConsumer(
        "kyverno",
        "argocd/platform/policy/applications/controller.yaml",
        "kyverno",
    ),
    ArgoConsumer(
        "grafana",
        "argocd/platform/observability/applications/grafana.yaml",
        "grafana",
    ),
    ArgoConsumer(
        "victoria_metrics_single",
        "argocd/platform/observability/applications/victoria-metrics.yaml",
        "victoria-metrics-single",
    ),
    ArgoConsumer(
        "loki",
        "argocd/platform/observability/applications/loki.yaml",
        "loki",
    ),
    ArgoConsumer(
        "tempo",
    ...
    ArgoConsumer(
        "reloader",
        "argocd/platform/core/applications/reloader.yaml",
        "reloader",
    ),
)
...
def chart_sources(document: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(sources, list):
        return [source for source in sources if isinstance(source, dict) and source.get("chart")]
...
def validate_renovate_annotations(path: Path, chart_keys: set[str]) -> list[str]:
    ...
    for key, annotation in sorted(annotations.items()):
        ...
        if datasource == "helm" and registry_url is None:
            errors.append(f"{VERSIONS_FILE}: Helm chart {key!r} must declare registryUrl")
        if datasource == "helm" and versioning != "helm":
            errors.append(f"{VERSIONS_FILE}: Helm chart {key!r} must use helm versioning")
        if datasource == "docker" and versioning != "semver-coerced":
            errors.append(f"{VERSIONS_FILE}: OCI chart {key!r} must use semver-coerced versioning")
    ...
    for chart_index, raw_line in enumerate(lines):
        ...
        for version_index in range(chart_index + 1, len(lines)):
            ...
            if revision_match:
                ...
                replacement = f'{revision_match.group(1)}targetRevision: "{version}"{newline}'
...
def validate(root: Path, write: bool) -> int:
    ...
    expected_keys = {consumer.key for consumer in (*TERRAFORM_CONSUMERS, *ARGO_CONSUMERS)}
    ...
    for path, chart in sorted(actual_tf - expected_tf):
        errors.append(f"{path}: Terraform chart {chart!r} is not registered in {VERSIONS_FILE}")
    ...
    for path, chart in sorted(actual_argo - expected_argo):
        errors.append(f"{path}: Argo CD chart {chart!r} is not registered in {VERSIONS_FILE}")
    ...
    for consumer in ARGO_CONSUMERS:
        ...
        matching_sources = [
            source for source in chart_sources(document) if source.get("chart") == consumer.chart
        ]
        if len(matching_sources) != 1:
            errors.append(f"{consumer.path}: expected exactly one {consumer.chart!r} source")
            continue
    ...
    if changed:
        ...
        print("[chart-versions] passed: all Terraform and Argo CD chart pins match versions.yaml")
    return 0
