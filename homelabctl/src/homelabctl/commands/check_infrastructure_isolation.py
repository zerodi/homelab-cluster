...
TASK_VARIABLES_SCRIPT = (
    PROJECT_ROOT / "homelabctl" / "src" / "homelabctl" / "commands" / "operations.py"
)
...
APPROVED_RESOURCES = {
    ...
    ("kubernetes_manifest", "openbao_ingress_policy"),
...
def strip_line_comment(line: str) -> str:
    ...
    while index < len(line):
        ...
        if escaped:
            ...
            not in_string
            and char == "#"
            or (not in_string and char == "/" and index + 1 < len(line) and line[index + 1] == "/")
        ):
            return line[:index]
        index += 1
...
def approved_runtime_identifier(path: Path, text: str, offset: int) -> bool:
    ...
    return (
        path.name == "providers.tf" and "local.environment_contract.hosts.authentik" in line
    ) or (
        path.name == "argocd.tf" and re.search(r'^\s*name\s*=\s*"Authentik"\s*$', line) is not None
    )
    ...
    for pattern, message in FORBIDDEN_SOURCE_PATTERNS:
        for match in pattern.finditer(active):
            if (
                ...
                continue
            violations.append(Violation(path, line_number(active, match.start()), message))
    ...
    if not variable_match:
        ...
        violations.append(
            Violation(
                ...
                "cluster_state_path must default to ../cluster/terraform.tfstate",
            )
        )
    ...
    if not re.search(
        ...
        violations.append(
            Violation(
                display_path,
                1,
                "cluster outputs must come from data.terraform_remote_state.cluster.outputs",
            )
        )
...
def scan_task_contract(text: str, display_path: Path) -> list[Violation]:
    ...
    required_patterns = (
        (
            r"homelabctl internal task-var environment-contract|def render_environment",
        ...
        (
            (
                ...
                r"cluster_output worker_hostnames json|task-var piraeus-nodes)"
            ),
...
def run_checks(infrastructure_dir: Path, display_root: Path) -> list[Violation]:
    ...
    for path in infrastructure_dir.rglob("*"):
        if path.is_file() and ".terraform" not in path.parts and path.suffix in HELPER_SUFFIXES:
            violations.append(
                Violation(
                    path.relative_to(display_root),
                    1,
                    "helper code must live in the repository homelabctl package",
                )
            )
...
PIRAEUS_NODES:
  sh: uv run --project homelabctl --locked homelabctl internal task-var piraeus-nodes
"""
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            f"[infrastructure-isolation] failed with {len(violations)} violation(s)",
            file=sys.stderr,
        )
        return 1
