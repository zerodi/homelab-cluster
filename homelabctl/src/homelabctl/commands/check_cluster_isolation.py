...
def scan_terraform_file(path: Path, display_root: Path) -> list[Violation]:
    ...
    for pattern, message in FORBIDDEN_SOURCE_PATTERNS:
        for match in pattern.finditer(text):
            violations.append(Violation(display_path, line_number(text, match.start()), message))

    for match in re.finditer(r'(?m)^\s*(variable|output)\s+"([^"]+)"\s*\{', text):
...
def declared_cluster_variables(cluster_dir: Path) -> set[str]:
    ...
    for path in sorted(cluster_dir.glob("*.tf")):
        ...
        declared.update(
            match.group(1) for match in re.finditer(r'(?m)^\s*variable\s+"([^"]+)"\s*\{', text)
        )
    return declared
...
def self_test() -> int:
    ...
    for sample, label in forbidden_samples.items():
        ...
        if not detected:
            block_match = re.search(r'(?m)^\s*(variable|output)\s+"([^"]+)"\s*\{', active)
            detected = bool(block_match and name_crosses_boundary(block_match.group(2)))
        if not detected:
    ...
    if assignments != ["proxmox", "argocd_host"]:
        failures.append(f"incorrect top-level tfvars assignments detected: {assignments!r}")
...
def main() -> int:
    ...
    tfvars_files = tuple(path.resolve() for path in (args.tfvars or DEFAULT_TFVARS_FILES))
