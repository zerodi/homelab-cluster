...
def load_contract() -> dict[str, Any]:
    return load(ROOT / VERSIONS_FILE)
...
def validate(write: bool) -> int:
    ...
    expected_keys = {name for consumers in GENERATED_CONFIGS.values() for name, _ in consumers}
    ...
    if not re.fullmatch(r"v[0-9]+\.[0-9]+\.[0-9]+", str(platform.get("talos_linux", ""))):
        errors.append(f"{VERSIONS_FILE}: platform.talos_linux must be vX.Y.Z")
    if not re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", str(platform.get("kubernetes", ""))):
    ...
    if match is None:
        ...
        if write:
            ...
            errors.append(f".github/workflows/ci.yaml: tofu_version must be {expected_tofu}")
