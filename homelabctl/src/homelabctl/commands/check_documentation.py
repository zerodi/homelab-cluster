...
def validate_layout() -> list[str]:
    ...
    for path in sorted(DOCS_ROOT.iterdir()):
        if path.is_dir():
            errors.append(
                f"{path.relative_to(PROJECT_ROOT)}: documentation must use the flat docs/ layout"
            )
    return errors
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        for match in LINK_PATTERN.finditer(line):
            ...
            if target.startswith("/"):
                errors.append(f"{relative_path}:{line_number}: absolute local link {target}")
                continue
            ...
            try:
                ...
                errors.append(f"{relative_path}:{line_number}: link leaves repository: {target}")
                continue
            if not resolved.exists():
                errors.append(f"{relative_path}:{line_number}: missing link target {target}")
...
def main() -> int:
    ...
    print("[documentation] passed: layout and local Markdown links are consistent")
    return 0
