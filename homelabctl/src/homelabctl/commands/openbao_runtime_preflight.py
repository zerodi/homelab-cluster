...
BCRYPT_HTPASSWD_PATTERN = re.compile(r"^[^:\r\n]+:\$2[aby]\$\d{2}\$[./A-Za-z0-9]{53}$")
...
def validate_property_shape(path: str, key: str, value: Any, validator: str) -> str | None:
    ...
    if validator == "bcrypt_htpasswd":
        if isinstance(value, str) and value.strip().startswith("REPLACE_WITH_"):
            return "still contains a REPLACE_WITH_* placeholder"
        if not isinstance(value, str) or BCRYPT_HTPASSWD_PATTERN.fullmatch(value.strip()) is None:
...
def validate_internal_shape_contract() -> list[str]:
    ...
    for label, value in {
        ...
        "unsupported bcrypt variant": (f"harbor_registry_user:$2x$10${bcrypt_payload}"),
...
def main() -> int:
    ...
    manifest_drift = [
        f"internal shape contract: {issue}" for issue in validate_internal_shape_contract()
    ]
    ...
    for secret_path, properties in SECRET_CONTRACT.items():
        generator_contract[secret_path] = set(properties)
    ...
    if missing_paths or missing_keys or shape_errors or (args.require_final and provisional_paths):
