...
from homelabctl.yamlutil import deep_merge, dump, load
...
def materialize_contract(raw_contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing_subdomains:
        raise ContractError("service_subdomains is missing: " + ", ".join(missing_subdomains))
    ...
    for field, value in (
        ...
        if not isinstance(value, str) or re.fullmatch(technical_name_pattern, value) is None:
            raise ContractError(
                f"{field} must use lowercase ASCII letters, digits, dot, underscore, or hyphen"
            )
    if not isinstance(administrator_display_name, str) or not administrator_display_name.strip():
    ...
    contract["identity"]["administrator"]["display_name"] = administrator_display_name.strip()
    ...
    contract["platform"]["gateway"]["addresses"] = {
        service: str(network[offset]) for service, offset in SERVICE_ADDRESS_OFFSETS.items()
    }
...
def render_contract(base_path: Path, override_path: Path, output_path: Path) -> Path:
    base = load(base_path, round_trip=True)
    if not isinstance(base, dict):
        raise ContractError("base environment contract must be a YAML mapping")
    merged = base
    if override_path.exists():
        override = load(override_path, round_trip=True)
        if not isinstance(override, dict):
            raise ContractError("environment override must be a YAML mapping")
        merged = deep_merge(base, override)
    rendered = materialize_contract(dict(merged))
    atomic_write(output_path, dump(rendered))
    return output_path
