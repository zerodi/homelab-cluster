...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict):
        raise RuntimeError("effective environment contract must be a mapping")
    return contract
...
def oauth_application_id(body: str, expected_name: str) -> str | None:
    ...
    for match in re.finditer(
        ...
        item_html = body[item_start if item_start >= 0 else 0 : match.start()]
...
def create_or_rotate_application(
    ...
    if application_id and not rotate:
        _, detail = session.request(
            f"{ADMIN_APPLICATIONS_PATH}/oauth2/{application_id}"
        )
...
def read_openbao(path: str) -> tuple[dict[str, Any], int]:
    ...
    if not isinstance(data, dict):
        raise RuntimeError(f"OpenBao path has no KV v2 data: {path}")
    if not isinstance(version, int):
        raise RuntimeError(f"OpenBao path has no KV v2 version: {path}")
    return data, version
...
def main() -> int:
    ...
    try:
        ...
        if not changed:
            if (
                current.get("forgejo_client") == client_id
                and str(current.get("bootstrap_provisional", "false")).casefold()
                not in {"1", "true", "yes"}
            ):
        ...
        return 0
    except (KeyError, RuntimeError, subprocess.CalledProcessError, urllib.error.URLError) as exc:
