locals {
  ...
  network_prefix_length = split("/", var.cluster_ipv4_cidr)[1]
  gateway               = cidrhost(var.cluster_ipv4_cidr, 1)
  controlplane_vip      = cidrhost(var.cluster_ipv4_cidr, 200)
  cilium_lb_pool_start  = cidrhost(var.cluster_ipv4_cidr, 230)
  cilium_lb_pool_stop   = cidrhost(var.cluster_ipv4_cidr, 250)

  controlplane_nodes = {
    for index in range(var.controlplane_nodes) :
    format("cp-%02d", index + 1) => {
      vm_id       = 9001 + index
      ip          = cidrhost(var.cluster_ipv4_cidr, 11 + index)
  ...
  worker_nodes = {
    ...
    format("wk-%02d", index + 1) => {
      vm_id       = 9101 + index
      ip          = cidrhost(var.cluster_ipv4_cidr, 101 + index)
